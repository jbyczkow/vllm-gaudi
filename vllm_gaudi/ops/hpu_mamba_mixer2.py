...
from vllm_gaudi.ops.causal_conv1d_pytorch_fp32 import (
...
class HPUMambaMixer2(MambaMixer2):
    ...
    def forward(
        ...
        # Create index mapping on CPU
        batch_indices_cpu = torch.repeat_interleave(
            torch.arange(batch_size, dtype=torch.int64),
            seq_lens_cpu
        )
        token_indices_list = []
        for seq_len in seq_lens_cpu:
            token_indices_list.append(torch.arange(seq_len.item(), dtype=torch.int64))
        token_indices_cpu = torch.cat(token_indices_list) if token_indices_list else torch.tensor([], dtype=torch.int64)
        
        # Move to HPU
        batch_indices = batch_indices_cpu.to(hidden_states.device)
        token_indices = token_indices_cpu.to(hidden_states.device)
    ...
    def conv_ssm_forward(
        ...
        num_prefills = int(additional_data[0].item())
        num_decodes = int(additional_data[1].item())
        num_prefill_tokens = int(additional_data[2].item())
        num_decode_tokens = int(additional_data[3].item())
        ...
        if has_prefill:
            ...
            hidden_states_B_C_p = hpu_causal_conv1d_fn(
                x,
                self.conv_weights,
                self.conv1d.bias,
        ...
        if has_decode:
            ...
            # 2. Convolution sequence transformation
            hidden_states_B_C_d = hpu_causal_conv1d_update(
                hidden_states_B_C_d,
                conv_state,
                self.conv_weights,
                self.conv1d.bias,
