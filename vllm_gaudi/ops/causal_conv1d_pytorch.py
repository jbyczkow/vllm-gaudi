...
def hpu_causal_conv1d_fn(
    ...
    if has_initial_state is not None:
        init_state = torch.where(has_initial_state, conv_states[batch_cache_idx, :, -state_len:], torch.zeros(padded_batch, dim, state_len, device=x_work.device, dtype=work_dtype))
    else:
